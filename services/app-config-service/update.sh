...
LAMBDA_CODE=AppConfigService-lambda.zip
...
eval FUNCTIONS=\$\("aws --region $MY_AWS_REGION lambda list-functions --query 'Functions[?starts_with(FunctionName, \`sb-${ENVIRONMENT}-app-config-\`)] | [].FunctionName' --output text"\)
FUNCTIONS=($FUNCTIONS)
