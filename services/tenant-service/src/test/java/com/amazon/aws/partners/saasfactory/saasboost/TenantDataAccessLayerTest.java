...
public class TenantDataAccessLayerTest {

    private static UUID tenantId;
    private static HashMap<String, String> attributes;
    private static HashMap<String, Tenant.Resource> resources;
    private static TenantAdminUser adminUser;
    private static Set<TenantAdminUser> users;
    ...
    public void testToAttributeValueMap() {
        ...
        expected.put("admin_users", AttributeValue.builder().l(users.stream()
                .map(user -> AttributeValue.builder().m(
                        Map.of(
                                "username", AttributeValue.builder().s(user.getUsername()).build(),
                                "email", AttributeValue.builder().s(user.getEmail()).build(),
                                "phone_number", AttributeValue.builder().s(user.getPhoneNumber()).build(),
                                "given_name", AttributeValue.builder().s(user.getGivenName()).build(),
                                "family_name", AttributeValue.builder().s(user.getFamilyName()).build()
                        )
                ).build())
                .collect(Collectors.toSet())).build()
        );

        Map<String, AttributeValue> actual = TenantDataAccessLayer.toAttributeValueMap(tenant);
        ...
        json.keySet().stream()
                ...
                .forEach(key -> {
            assertTrue(actual.containsKey(key), "Class property '" + key + "' does not exist in DynamoDB attribute map");
        });
    }

    @Test
    public void testMapAttributeExpressionName() {
        assertThrows(IllegalArgumentException.class, () -> TenantDataAccessLayer.mapAttributeExpressionName(null));
        assertThrows(IllegalArgumentException.class, () -> TenantDataAccessLayer.mapAttributeExpressionName(""));
        assertThrows(IllegalArgumentException.class, () -> TenantDataAccessLayer.mapAttributeExpressionName("  "));

        assertEquals(TenantDataAccessLayer.mapAttributeExpressionName("ECS_CLUSTER"), "#ECS_CLUSTER");
    }

    @Test
    public void testMapAttributeExpressionValue() {
        assertEquals(TenantDataAccessLayer.mapAttributeExpressionValue("PRIVATE_SUBNET_A"),
                ":PRIVATE_SUBNET_A");
    }

    @Test
    public void testMapAttributeUpdateExpression() {
        assertEquals(TenantDataAccessLayer.mapAttributeUpdateExpression("resources", "VPC", "VPC"),
                "resources.#VPC = :VPC");
    }
}
