...
LAMBDA_CODE=SystemUserService-lambda.zip
...
eval FUNCTIONS=\$\("aws --region $MY_AWS_REGION lambda list-functions --query 'Functions[?starts_with(FunctionName, \`sb-${ENVIRONMENT}-sys-user-\`)] | [].FunctionName' --output text"\)
FUNCTIONS=($FUNCTIONS)
