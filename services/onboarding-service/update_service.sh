...
LAMBDA_CODE=OnboardingService-lambda.zip
...
eval FUNCTIONS=\$\("aws --region $MY_AWS_REGION lambda list-functions --query 'Functions[?starts_with(FunctionName, \`sb-${ENVIRONMENT}-onboarding-\`)] | [].FunctionName' --output text"\)
FUNCTIONS=($FUNCTIONS)
