...
LAMBDA_CODE=CustomizeCognitoUi-lambda.zip
...
FUNCTION="sb-${ENVIRONMENT}-customize-cognito-ui"
printf "Updating function code for ${FUNCTION}\n"
aws lambda --region "$MY_AWS_REGION" get-function --function-name "$FUNCTION"
if [ $? -e 0 ]; then
